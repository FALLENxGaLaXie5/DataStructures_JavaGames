...
public class IntervalSearch
{
     ...
     * set found to false
     * set foundItemIndex to -1
     * while found is false and first is less than last
     *      set mid to the midpoint
     *      if the desiredItem is equal to the midPoint data
     *          set found to true
     *          set foundItemIndex to mid
     *      else if the desired item is less than the midpoint item
     *          set last to mid - 1
     *      else
     *          set first to mid + 1
     * @param sortedData
    ...
    int binarySearch(T[] sortedData, int first, int last, T desiredItem)
    {
        boolean found = false;
        ...
        while (!found && first <= last)
        {
            ...
            if (desiredItem.equals(sortedData[mid]))
            {
                found = true;
                foundItemIndex = mid;
            }
            else if (sortedData[last].compareTo(desiredItem) < 0)
            {
                found = true;
                foundItemIndex = last;
            }
            else if (sortedData[first].compareTo(desiredItem) > 0)
            {
                found = true;
                foundItemIndex = first;
            }
            else if (desiredItem.compareTo(sortedData[mid]) < 0)
            {
                last = mid - 1;
            }
            else if (desiredItem.compareTo(sortedData[mid]) > 0)
     ...
     * set rightBoundary to the length of the sorted data
     * if the length of sortedData is greater than 0 and the size of targetValues is greater than 0
     *      set smallestTarget to the first value of targetValues
     *      set the largestTarget to the last value of targetValues
     *      if the smallestTarget equals the firstEntry in sortedData
     *          set the leftBoundary to 0
     *      else if left than firstEntry
     *          set to -1
     *      else call binarySearch to find smallestTarget
     *
     *
     *      repeat for largest target
     * @param sortedData
     * @param targetValues
     * @param <T>
     * @return
	...
	Interval findInterval(T[] sortedData, TreeSet<T> targetValues)
	{
        ...
        int foundLeft = leftBoundary;
        int foundRight = rightBoundary;
		...
		return new Interval(foundLeft, foundRight);
	} // end findInterval
